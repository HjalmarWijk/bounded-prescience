...
def check_noops(env_str,method_str,prop_str,action_function,max_noops=29,render = False,max_frames = 100000,verbose = False, min_noops=0, shield = 0,demand_full_safety=False,record=False): 
    ...
    for i in range(min_noops,max_noops):
        ...
        checker = ModelChecker(action_function,env,env.action_space.n,i,shield=shield,render=render,verbose=verbose,max_frames=max_frames)
        output = None
        while output == None:
            output = checker.step()
...
class ModelChecker():
    ...
    def evaluate_step(self):
        ...
        if (self.frame > self.noops and self.violation != "None") or (self.frame <= self.noops and self.noop_violation!="None"):
            info['label'] = False
    ...
    def step(self):
        ...
        if unsafe and self.shield > 0:
            ...
            self.take_step(obs,reward) #takes the evaluated step, but stores the state and can be retracted until the step is committed
